/*
# ElevateX — Institutional Trading Platform Schema

1. Overview
Creates the complete database schema for ElevateX, a premium institutional
trading analysis platform with user auth, markets, analysis (daily/weekly/monthly
with full SMC data), chart galleries, blog, trading journals, notifications,
social links, and support tickets.

2. New Tables
- profiles: extends auth.users (full_name, avatar, role, plan, bio)
- markets: trading pairs with category, logo, price fields, trends, biases
- analysis: analysis posts with full SMC fields (zones, entry, SL, TPs, RR, confidence, sentiment)
- charts: chart gallery images attached to analysis/market
- blog_posts: articles with category, cover, author, status
- journal_trades: user trading journal (owner-scoped)
- notifications: user in-app notifications (owner-scoped)
- social_links: admin-managed social links (public read)
- support_tickets: contact/support tickets

3. Security
- RLS on every table.
- profiles: owner read/update; admin read all.
- markets, charts, social_links: public read; admin/analyst write.
- analysis, blog_posts: public read published; admin/analyst write.
- journal_trades, notifications: owner-scoped CRUD.
- support_tickets: anon+auth insert; owner+admin read.
*/
